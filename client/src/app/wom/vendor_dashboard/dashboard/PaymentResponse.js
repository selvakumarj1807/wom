...
const UserAcknowledge = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.get(`https://wom-server.onrender.com/api/v1/admin/orderManagementVendor/orderByEmail/${emailCookie}`);
                ...
                {data.length > 0 ? (
                  data.map((elem, index) => (
                    <tr>
                      ...
                      <td style={{ wordWrap: 'break-word', color: 'orange', alignContent: 'center', textAlign: 'center' }}><a href={`/vendor/viewProducts/${elem._id}`}>View Products</a></td>
