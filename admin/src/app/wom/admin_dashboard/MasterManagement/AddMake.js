...
const AddMake = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const fetchedData = response.data.addMake;
    ...
    const Submit = async (e) => {
        ...
        try {
            if (editMode) {
                const response = await axios.put(`https://wom-server.onrender.com/api/v1/masterManagement/addMake/${selectedId}`, { make });
                if (response.status === 200) {
                    setSuccessMessage('Successfully updated!');
                }
            } else {
                const response = await axios.post("https://wom-server.onrender.com/api/v1/masterManagement/addMake/new", { make });
    ...
    const handleDelete = (id) => {
        axios.delete("https://wom-server.onrender.com/api/v1/masterManagement/addMake/" + id)
            .then(res => {
                console.log(res)
                            ...
                            {data.length > 0 ? (
                                data.map((elem, index) => (
                                    <tr key={elem._id}>
                                        ...
                                        <td>{elem.make}</td>
                                        <td>
                                            <button style={styles.editButton} onClick={() => openEditModal(elem._id, elem.make)}>Edit</button>
