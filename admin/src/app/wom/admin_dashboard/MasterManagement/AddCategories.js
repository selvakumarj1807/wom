...
const AddCategory = () => {
    ...
    const Submit = async (e) => {
        ...
        try {
            if (editMode) {
                const response = await axios.put(`https://wom-server.onrender.com/api/v1/masterManagement/addCategory/${selectedId}`, { category });
                if (response.status === 200) {
                    setSuccessMessage('Successfully updated!');
                }
            } else {
                const response = await axios.post("https://wom-server.onrender.com/api/v1/masterManagement/addCategory/new", { category });
    ...
    const handleDelete = (id) => {
        axios.delete("https://wom-server.onrender.com/api/v1/masterManagement/addCategory/" + id)
            .then(res => {
                console.log(res)
    ...
    const openEditModal = (id, category) => {
    ...
    return (
        <div id="main" className="main" style={{ padding: '20px' }}>
            ...
            <h2 style={{ textAlign: 'center', color: 'rgb(14, 42, 71)' }}>Add Category For Vendor Product</h2>
                            ...
                            {data.length > 0 ? (
                                data.map((elem, index) => (
                                    <tr key={elem._id}>
                                        ...
                                        <td>{elem.category}</td>
                                        <td>
                                            <button style={styles.editButton} onClick={() => openEditModal(elem._id, elem.category)}>Edit</button>
